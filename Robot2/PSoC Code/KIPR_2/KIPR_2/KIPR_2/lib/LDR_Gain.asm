...
;  Global Symbols
;-----------------------------------------------
export  LDR_Gain_Start
export _LDR_Gain_Start
export  LDR_Gain_SetPower
export _LDR_Gain_SetPower

export  LDR_Gain_SetGain
export _LDR_Gain_SetGain

export  LDR_Gain_Stop
export _LDR_Gain_Stop

IF (LDR_Gain_AGNDBUFAPI)
export  LDR_Gain_EnableAGNDBuffer
export _LDR_Gain_EnableAGNDBuffer
export  LDR_Gain_DisableAGNDBuffer
export _LDR_Gain_DisableAGNDBuffer
ENDIF
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: LDR_Gain_Start
;  FUNCTION NAME: LDR_Gain_SetPower
;
;  DESCRIPTION:
...
;
   ...
   mov  A, reg[LDR_Gain_GAIN_CR2]                          ; read power value
   and  A, ~POWERMASK                                ; clear power bits in A
   or   A, [X]                                       ; combine power value with balance of reg.
   mov  reg[LDR_Gain_GAIN_CR2], A                     ; move complete value back to register
   pop  A
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: LDR_Gain_SetGain
;
;  DESCRIPTION:
...
;
   ...
   mov  A, reg[LDR_Gain_GAIN_CR0]                          ; read mux settings
   and  A, ~GAINREGMASK                                   ; clear gain bits in A
   tst  [X],HIGHGAIN                                      ; See if High Gain is set
   jnz  .SETHIGHGAIN
   and  reg[LDR_Gain_GAIN_CR3],~EXGAIN                     ; Clear High Gain bit.
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[LDR_Gain_GAIN_CR0], A                          ; move complete value back to register
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
...
.SETHIGHGAIN:
   ...
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[LDR_Gain_GAIN_CR0], A                          ; move complete value back to register
   or   reg[LDR_Gain_GAIN_CR3], EXGAIN                     ; Set High Gain bit.
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
.ENDSECTION
...
_LDR_Gain_Stop:
   ...
   and REG[LDR_Gain_GAIN_CR2], ~POWERMASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: LDR_Gain_EnableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 LDR_Gain_EnableAGNDBuffer:
_LDR_Gain_EnableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   reg[LDR_Gain_GAIN_CR3], ~AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: LDR_Gain_DisableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 LDR_Gain_DisableAGNDBuffer:
_LDR_Gain_DisableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   or    reg[LDR_Gain_GAIN_CR3], AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
ENDIF

; End of File LDR_Gain.asm
